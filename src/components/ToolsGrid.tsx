...
import { Calculator, BookOpen, Newspaper, GraduationCap, FileText, CheckSquare, Search, Award } from 'lucide-react';
...
const ToolCard: React.FC<ToolCardProps> = ({ title, subtitle, icon: Icon, path, delay = 0 }) => {
  return (
    <Link to={path} className="block group">
      <motion.div 
        ...
        className="h-full bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-xl hover:border-blue-500/30 dark:hover:border-blue-500/30 transition-all duration-300 relative overflow-hidden"
      >
        ...
        <div className="relative z-10 flex items-start gap-4">
          <div className="w-14 h-14 rounded-xl bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0 shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
            <Icon size={28} strokeWidth={1.5} />
          </div>
          
          <div>
            <h3 className="font-bold text-gray-900 dark:text-white text-lg leading-tight mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
              {title}
            </h3>
            <div className="text-xs font-mono text-gray-500 dark:text-gray-400 mb-2 truncate max-w-[200px]">
              campusai.com.ng{path}
            </div>
            <div className="inline-flex items-center text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-full">
              {subtitle}
            </div>
          </div>
        </div>
